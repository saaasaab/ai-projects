...
function sampleT(index: number, count: number): number {
...
export function sampleAboveBelow(
  ...
  return {
    above: sampleAlongEdge(imageData, sourceBounds, imageWidth, imageHeight, strip, "top", sampleOffset, smoothing),
    below: sampleAlongEdge(imageData, sourceBounds, imageWidth, imageHeight, strip, "bottom", sampleOffset, smoothing),
  };
}

export function sampleAlongEdge(
  imageData: ImageData,
  sourceBounds: Bounds,
  imageWidth: number,
  imageHeight: number,
...
export function pickAutoEdgeColor(
  imageData: ImageData,
  sourceBounds: Bounds,
  imageWidth: number,
  imageHeight: number,
  ...
  const edgeGroups = new Map<SampleEdge, SamplePoint[]>();
  for (const point of points) {
    const list = edgeGroups.get(point.edge) ?? [];
    list.push(point);
    edgeGroups.set(point.edge, list);
  }

  let bestEdge: SampleEdge | null = null;
  let lowestVariance = Infinity;
  let bestColor: RGB = { r: 0, g: 0, b: 0 };

  for (const [edge] of edgeGroups) {
    const colors: RGB[] = [];
    for (let i = 0; i < AUTO_VARIANCE_SAMPLES; i++) {
      const t = sampleT(i, AUTO_VARIANCE_SAMPLES);
...
export async function loadImageData(pngBytes: number[]): Promise<{ imageData: ImageData; width: number; height: number }> {
